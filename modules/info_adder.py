...
def get_trashing_quality(df):
    ...
    df.loc[(~df["Trash / Return"].str.lower().apply(lambda x: x in ["self", "self?"])) & (df["Trash / Return"].str.len()
                                                                                          != 0), "ThinningQualityTest"]=10  # The self-trashers are only returned, so we don't want them in here
    for i in range(5):
        df.loc[(df["Trash / Return"].str.contains(str(i))),
               "ThinningQualityTest"] = i * 2
        df.loc[(df["Exile"].str.contains(str(i))),
               "ThinningQualityTest"] = i * 2
    my_trashers = df[["Name", "ThinningQualityTest"]].to_dict()
    my_trashers = {my_trashers["Name"][i]: my_trashers["ThinningQualityTest"][i]
...
def get_attack_types(df):
    attack_dict = {}
    attack_dict["Handsize"] = ["Bureaucrat", "Militia", "Minion", "Torturer", "Cutpurse",
                "Ghost Ship", "Goons", "Followers", "Margrave", "Urchin", "Pillage",
                "Sir Michael", "Knights", "Mercenary", "Taxman", "Relic", "Soldier",
                "Catapult", "Catapult/Rocks", "Legionary", "Raider", "Villain",
                "Skirmisher", "Archer", "Clashes", "Haunted Woods", "Gang of Pickpockets"]
    attack_dict["Trashing"] = ["Swindler", "Thief", "Bandit", "Saboteur", "Pirate Ship",
                       "Noble Brigand", "Rogue", "Dame Anna", "Dame Josephine",
                       "Dame Molly", "Dame Natalie", "Dame Sylvia", "Sir Bailey",
                       "Sir Destry", "Sir Martin", "Sir Michael", "Sir Vander",
                       "Knights", "Giant", "Warrior", "Cardinal", "Barbarian"]
    attack_dict["Junking"] = ["Swindler", "Witch", "Replace", "Torturer", "Ambassador", "Sea Hag",
              "Familiar", "Mountebank", "Young Witch", "Jester", "Noble Brigand",
              "Marauder", "Cultist", "Soothsayer", "Giant", "Catapult", "Catapult/Rocks",
              "Idol", "Old Witch", "Black Cat", "Coven", "Barbarian", "Sorcerer",
              "Sorceress", "Wizards", "Augurs", "Circle of Witches"]
    attack_dict["Deck order"] = ["Bureaucrat", "Spy", "Sea Hag", "Scrying Pool",
                                 "Ghost Ship", "Rabble", "Haunted Woods",
                  "Fortune Teller", "Jester", "Oracle",
    ...
    attack_dict["Turn worsening"] = ["Bridge Troll", "Swamp Hag", "Relic", "Enchantress",
                   "Gatekeeper", "Highwayman", "Warlord", "Clashes"]
    attack_dict["Doom"] = list(df[df["Types"].apply(lambda x: "Doom" in x)]["Name"])
    attack_dict["Doom"] = [att for att in attack_dict["Doom"] if att not in ["Leprechaun", "Cursed Village"]]
    attack_dict["Scoring"] = ["Witch", "Swindler", "Replace", "Torturer", "Sea Hag", "Familiar",
               "Mountebank", "Young Witch", "Jester", "Followers", "Giant",
               "Catapult", "Catapult/Rocks", "Idol", "Old Witch", "Black Cat",
               "Coven", "Barbarian", "Swamp Hag", "Wizards", "Augurs",
               "Circle of Witches"]
    cards = {}
    attack_dict["Indirect"] = ["Possession", "Raid", "Ill-Gotten Gains",
                               "Masquerade", "Haunted Castle"]
