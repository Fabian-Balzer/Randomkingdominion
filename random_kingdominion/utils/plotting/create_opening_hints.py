...
def _annotate_single_coin(ax: Axes, coin_type: str, ycoord: float, x_offset=0.0):
...
def _annotate_coin_icons(ax: Axes, open_type: str, y0: float):
    ...
    ax.add_patch(rect)
    line = Line2D(
        [box_x + xwidth / 2 * 0.95, box_x + xwidth / 2 * 1.15],
        [box_y, box_y + yheight],
        color="black",
        linewidth=2,
    )
...
def _annotate_cso_imgs(
    ax: Axes, x0: float, y0: float, csos: list[str], num_openings: int
):
...
def _annotate_text_and_imgs_for_turn(
    ax: Axes,
    x0: float,
    y0: float,
    turn: Literal[1, 2],
...
def _annotate_single_opening(
    ...
    _annotate_text_and_imgs_for_turn(
        ax, x1, ycoord - 0.01, 1, opening_info, num_openings
    )
    _annotate_text_and_imgs_for_turn(
        ax, x2, ycoord - 0.01, 2, opening_info, num_openings
    )
...
def create_opening_hints(k: "Kingdom"):
    ...
    assert all(
        [
            all([x in subdict.keys() for x in ["grade", "type", "t1", "t2"]])
            for subdict in openings
        ]
    ), f"Some part of the openings are missing information ({openings})"
    assert all(
        [subdict.get("grade") != "" for subdict in openings]
    ), "Some openings are missing a grade"
    ...
    fig.savefig(
        PATH_ASSETS.joinpath("other/youtube/current_openings.png"),
        dpi=300,
        bbox_inches="tight",
    )
